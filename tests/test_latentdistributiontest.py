...
def test_n_bootstraps():
    ...
    _, _, null_distribution_ = latent_position_test(A1, A2, n_bootstraps=123)
...
def test_SBM_dcorr():
    ...
    A2 = sbm(2 * [b_size], B1)
    A3 = sbm(2 * [b_size], B2)
    p_null = latent_distribution_test(A1, A2)
...
def test_different_aligners():
    ...
    p_val_1, _, _ = latent_distribution_test(A1, A2, input_graph=False, align_type=None)
    assert p_val_1 < 0.05

    p_val_2 = latent_distribution_test(
