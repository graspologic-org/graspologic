...
from graspy.plot.plot import heatmap, gridplot, pairplot, pairplot_with_gmm, _sort_inds
...
def test_pairplot_with_gmm_inputs_():
    r"""
    Function to test inputs of pairplot_with_gmm
    """
    X = np.random.rand(15, 3)
...
def test_pairplot_with_gmm_outputs():
    r"""
    Function to test outputs of pairplot_with_gmm
    """
    X = np.random.rand(15, 3)
    gmm = GaussianMixture(n_components=3, covariance_type='full').fit(X)
    labels = ["A"] * 5 + ["B"] * 5 + ["C"] * 5
