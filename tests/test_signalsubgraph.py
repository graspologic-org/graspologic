...
class TestVertexScreener(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        cls.n = [20,80]
        cls.p = [0.3,0.4]
        cls.P0 = np.array([cls.p[0],.2,.2,.3]).reshape(K,K)
        cls.P1 = np.array([cls.p[1],.2,.2,.3]).reshape(K,K)
    ...
    def test_vertexscreener_sbm(self):
        ...
        self.assertTrue(checkEqual(classifier.vertices_of_interest,true_verticesofinterest))
