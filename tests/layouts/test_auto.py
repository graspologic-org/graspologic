...
from graspologic.layouts.auto import (
    _get_bounds,
    embed_for_layout,
    get_node_positions,
...
class TestAuto(unittest.TestCase):
    ...
    def test_layout_string_node_ids(self):
        graph = nx.florentine_families_graph()

        for s, t in graph.edges():
            graph.add_edge(s, t, weight=1)

        lcc_graph = preprocess_for_layout(graph=graph)
        tensors, labels = embed_for_layout(graph=lcc_graph)
        points_2d = reduce_dim_for_layout(embedding=tensors)
        node_positions = get_node_positions(
            graph=lcc_graph, points_2d=points_2d, labels=labels
        )

        self.assertEqual(len(node_positions), len(graph.nodes()))

    def test_layout_int_node_ids(self):
        graph = nx.florentine_families_graph()
        ...
        lcc_graph = preprocess_for_layout(graph=graph_int_node_ids)
        tensors, labels = embed_for_layout(graph=lcc_graph)
        points_2d = reduce_dim_for_layout(embedding=tensors)
        node_positions = get_node_positions(
            graph=lcc_graph, points_2d=points_2d, labels=labels
    ...
    def test_layout_directed_weighted(self):
