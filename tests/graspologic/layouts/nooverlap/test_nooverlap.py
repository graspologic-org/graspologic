...
class TestNoOverlap(unittest.TestCase):
	...
	def test_find_grid_cell_and_center(self):
		x, y = 2, 5
		cell_x, cell_y, cell_center_x, cell_center_y = self.qn.find_grid_cell_and_center(self.min_x, self.min_y, self.max_size, x, y)
	...
	def test_find_free_cell(self):
		cells ={}
		result = self.qn.find_free_cell(cells, self.x, self.y, self.num_x, self.num_y, self.min_x, self.min_y, self.max_size)
...
##
	def test_find_free_cell_20_used(self):
		...
		cells[(0,2)] = 999
		cells[(0,1)] = 999
		result = self.qn.find_free_cell(cells, 0, 2, self.num_x, self.num_y, self.min_x, self.min_y, self.max_size)
