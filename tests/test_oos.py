...
def test_fit_predict():
    ...
    ase_object1 = OutOfSampleAdjacencySpectralEmbed(semi_supervised=True,
                                                    in_sample_idx = range(int(np.ceil(0.8*n)))
                                                )
    ase_object1.fit(A)
    ...
    np.random.seed(8888)
    ase_object2 = OutOfSampleAdjacencySpectralEmbed(semi_supervised=True,
                                                    in_sample_idx = range(int(np.ceil(0.8*n)))
                                                )
