...
def construct_contingency(graphs, labels):
    ...
    if not isinstance(labels, (list, np.ndarray)):
        msg = "Input vector 'labels' must be list or np.ndarray, not {}.".format(type(labels))
        raise TypeError(msg)
    ...
    if len(np.unique(labels)) > 2:
        msg = "Input arrays must have a maximum of two classes, not {}.".format(len(np.unique(labels)))
        raise ValueError(msg)
...
def estimate_signal_subgraph(graphs, labels, constraints):
    ...
    sigmat = np.array([[fisher_exact(tables[i,j,:,:])[1] for j in range(verts)] for i in range(verts)])
    
    if isinstance(constraints, (int, float)): #incoherent
        nedges = constraints
        sigsub = np.dstack(np.unravel_index(np.argsort(sigmat.ravel()), np.shape(sigmat)))
        ...
        while wconv == 0:
            ...
            if (vscore[:nverts].sum()) >= nedges:
                ...
                indsp = np.dstack(np.unravel_index(np.argsort(blank.ravel()), np.shape(blank)))
