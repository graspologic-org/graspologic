...
class SparseOpt(BaseClassify):
    ...
    def _admm(self, omega1, omega2, tol=1e-7, beta_start=None):
        ...
        v = np.zeros(m)
        i = 1
        phi_beta_k = (0.5*np.sum(self.y - beta @ self.y - beta) + omega1*np.sum(np.abs(beta))
        ...
        while (resk > tol or sk > tol) and i <= self.max_iter:
            ...
            phi_beta_k1 = (0.5*np.sum(self.y - beta @ self.y - beta)
        ...
        phi_beta_q = (0.5*np.sum(self.y - beta_q @ self.y - beta_q)
    ...
    def fit(self, x, y, xtest):
        ...
        self._predict()
        yfit_test = xtest @ self.beta + self.b
