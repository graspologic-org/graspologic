...
class ItScreen(BaseSubgraph):
    ...
    def fit(self, X, y):
        ...
        if (self.subgraph_n_verts < 0 or
                self.subgraph_n_verts > self.n_vertices):
            msg = "subgraph_n_verts must be between 0 and n_vertices"
        ...
        val = 1 - self.delta
        while (val ** itr) * self.n_vertices > self.subgraph_n_verts:
