...
def latent_distribution_test(
    ...
    data = test_obj.test(
        X1_hat, X2_hat, reps=n_bootstraps, workers=workers, auto=False,
    )
...
def _embed(A1, A2, n_components, svd_seed=None):
...
def add_variance(X_orig, X_sigma, random_state=None):
    rng = check_random_state(random_state)
