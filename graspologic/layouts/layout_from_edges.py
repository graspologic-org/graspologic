...
def _partition_graph(graph: networkx.Graph):
    ...
    logger.info (f"nodes and edges in network: {graph.number_of_nodes()}, {graph.number_of_edges()}")
...
def _get_node_colors(color_file, partition, light_background=True, color_is_continuous=False, color_scheme='nominal', node_attributes=None):
    from ._helpers import get_node_colors_from_partition, create_colormap, read_json_colorfile, get_partition
    ...
    if color_is_continuous:
        if node_attributes is None:
            raise Exception("Need node_file, node_id, and color_attribute to specify continous value")
...
def layout_with_node2vec_umap(graph: networkx.Graph, min_dist: float = 0.75, n_neighbors: int = 25, max_edges=10000000):
    '''
    ...
    transformed_points = umap.UMAP(min_dist=min_dist, n_neighbors=neighbors).fit_transform(tensors)
    umap_time = time.time() - start
    logger.info (f"umap completed in {umap_time} seconds")

    nx_graph, positions = _make_graph_and_positions(labels, transformed_points, lcc.degree())
    non_overlapping_positions = remove_overlaps(positions)

    return nx_graph, non_overlapping_positions

def layout_with_node2vec_tsne(graph: networkx.Graph, perplexity: int, n_iters: int, max_edges_to_keep: int =10000000):
    '''
        Performs edge cuts to ge the number of edges under 1M edges.
        Finds the largeest connected component, performs a node2vec embedding on the LCC.
    ...
    start = time.time()
    transformed_points = TSNE(perplexity=perplexity, n_iter=n_iters).fit_transform(tensors)
    tsne_time = time.time() - start
    logger.info (f"tsne completed in {tsne_time} seconds with peplexity: {perplexity} and n_iters: {n_iters}")
...
def layout_node2vec_umap_from_file(edge_file: str,
    ...
    node_colors = _get_node_colors(color_file_json, partition, light_background=True, color_is_continuous=False, color_scheme='nominal', node_attributes=None)

    if location_file is not None:
        write_locations(location_file, positions, partition)
    if image_file is not None:
        save_graph(image_file, lcc, positions, node_colors=node_colors, dpi=dpi, light_background=True)
    return

def layout_node2vec_tsne_from_file(edge_file : str,
        image_file: Optional[str],
        location_file: Optional[str],
        dpi: int, perplexity: int=30,
        n_iters: int=1000,
        max_edges: int=10000000
        ):
    '''
    ...
    graph = read_graph(edge_file)
    print (f"nodes, edges after read: {graph.number_of_nodes()}, {graph.number_of_edges()}")
    lcc, positions = layout_with_node2vec_tsne(graph, perplexity, n_iters, max_edges_to_keep=max_edges)
    partition = _partition_graph(graph)
...
def remove_overlaps(list_of_node_positions: List[NodePosition] ):
    ...
    for n in local_nodes:
        new_positions.append(NodePosition(node_id=n.nid, x=n.x, y=n.y, size=n.size, community=n.community))
...
def _extract_positions_from_graph(graph, id_to_community):
    ...
    for key, att in graph.nodes(data=True):
        ...
        if key in id_to_community:
            community = id_to_community[key] if type(id_to_community[key]) == int else None
            positions.append(NodePosition(node_id=key, x=x, y=y, size=size, community=community))
...
def _cut_edges_if_needed(graph, max_edges_to_keep=1000000):
    ...
    if num_edges > max_edges_to_keep:
        ...
        new_graph = cut_edges_by_weight(graph, cut_threshold=edge_cut_weight, cut_process="smaller_than_inclusive")
...
def _new_bounds(minx, miny, maxx, maxy, covered_area):
    ...
    new_minx, new_miny, new_maxx, new_maxy = mid_x - new_range_x/2, mid_y - new_range_y/2, mid_x + new_range_x/2, mid_y + new_range_y/2
...
def _center_points_on_origin(points):
    ...
    minx, miny, maxx, maxy = minx - center_x, miny - center_y, maxx-center_x, maxy-center_y
    #print (minx, miny, maxx, maxy)
    return moved_points
...
def _scale_points(points, covered_area):
    ...
    new_minx, new_miny, new_maxx, new_maxy = _new_bounds(minx, miny, maxx, maxy, covered_area)
    ...
    final_locs = _scale_to_new_bounds(scaled_points, new_minx, new_miny, new_maxx, new_maxy)
...
def _make_graph_and_positions(vertex_labels, points, degrees):
    ...
    for idx, key in enumerate(vertex_labels):
        graph.add_node(key)
        positions.append(NodePosition(node_id=key, x=scaled_points[idx][0], y=scaled_points[idx][1], size=sizes[key], community=None))
...
def write_locations(filename, positions, partition):
    ...
    with open (filename, 'w') as ofile:
        ...
        for node in positions:
            writer.writerow([node.node_id, node.x, node.y, node.size, partition[node.node_id]])
