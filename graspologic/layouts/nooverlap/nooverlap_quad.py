...
def is_overlapping_any_node_and_index_with_grid(node, new_x, new_y, nodes, start, end, size, grid):
    overlapping = None
    idx = 0
...
class quad_node:
    ...
    def find_center(self):
        ...
        self.min_x, self.min_y, self.max_x, self.max_y, self.max_size = stats_nodes(self.nodes)
        ...
        self.total_cells = self.get_total_cells(self.min_x, self.min_y, self.max_x, self.max_y, self.max_size)
        if tot_area == 0:
            self.sq_ratio = 1.0
            self.cir_ratio = 1.0
        else:
    ...
    def push_to_kids(self):
        ...
        self.NW = quad_node(nw_nodes, self.depth+1, self.max_nodes_per_quad, self) if len(nw_nodes) > 0 else None
        self.NE = quad_node(ne_nodes, self.depth+1, self.max_nodes_per_quad, self) if len(ne_nodes) > 0 else None
        self.SW = quad_node(sw_nodes, self.depth+1, self.max_nodes_per_quad, self) if len(sw_nodes) > 0 else None
        self.SE = quad_node(se_nodes, self.depth+1, self.max_nodes_per_quad, self) if len(se_nodes) > 0 else None
    ...
    def get_stats_for_quad(self, max_depth, stats_list, magnification=10):
        ...
        #print ('-'*self.depth, "size: %d, (%g, %g), (%g, %g) cs: %g, tot: %g, ratio: %g, ss: %g, ratio: %g, tag: %s" %(len(self.nodes), self.min_x, self.min_y, self.max_x, self.max_y, self.circle_size, tot_area, self.circle_size/tot_area, self.square_size, self.square_size/tot_area, tag))
        if self.depth >= max_depth:
            stats_list.append([self.x, self.y, self.circle_size/tot_area*magnification, tag, self.square_size/tot_area*magnification])
            return
    ...
    def find_free_cell(self, cells, x, y, num_x_cells, num_y_cells, min_x, min_y, max_size):
        #zero the cordinates
        ...
        while (new_x, new_y) in cells or not done:
            ...
            for new_x in range(new_x+1, new_x+distance_to_move+1):
                ...
                if not (new_x, new_y) in cells and new_x < num_x_cells and new_y < num_y_cells and new_x >=0 and new_y >=0:
                    #print('breaking-r')
                    done = True
                    break
            if done:
    ...
    def layout_node_list(self,min_x, min_y, max_x, max_y, max_size, node_list ):
        ...
        if largest_size != max_size:
            raise Exception("This can not be!!! max: %g, largest: %g" %(max_size, largest_size))
        ...
        if num_nodes > total_cells:
            raise Exception("Too many nodes per Cell for this quad! nodes: %d, cells: %d" % (num_nodes, total_cells))
        ...
        for idx, node_to_move in enumerate(nodes_by_size):
            for placed_node in nodes_by_size[idx+1:]:
                if is_overlap(node_to_move.x, node_to_move.y, node_to_move.size, placed_node.x, placed_node.y, placed_node.size):
        ...
        for idx, node_to_move in enumerate(nodes_by_size):
            ...
            cell_x, cell_y, cell_center_x, cell_center_y = self.find_grid_cell_and_center(min_x, min_y, max_size, node_to_move.x, node_to_move.y)
            ...
            if found:
                ...
                cell_x, cell_y, cell_center_x, cell_center_y = self.find_free_cell(cells, cell_x, cell_y, number_of_x_cells, number_of_y_cells, min_x, min_y, max_size)
    ...
    def layout_quad(self):
        ...
        if not has_children:
            if num_nodes > self.total_cells:
                logger.info ("We don't fit! going up one level depth: %d, cells: %d, nodes %d, ratio: %g, max_size: %g" %(self.depth, self.total_cells, num_nodes, nodes_per_cell, self.max_size))
                parent = self.parent
                #if parent is not None:
                while parent is not None:
                    logger.info ('parent: sq_ratio: %g, cir_ratio: %g, cells %d, nodes: %d current_level %d, max_size: %g' % (parent.sq_ratio, parent.cir_ratio, parent.total_cells, len(parent.nodes), parent.depth, parent.max_size))
                    if  len(parent.nodes) > parent.total_cells:
                        #go up one more level
                        logger.info ('A Quad at level %d does not have enough space to layout its nodes' %(parent.depth))
                        parent = parent.parent
                    else:
                        #min_x, min_y, max_x, max_y, max_size = stats_nodes(parent.nodes)
                        #for n in parent.nodes:
                        #	n.color = '#FF0004'
                        overlapping = parent.layout_node_list(parent.min_x, parent.min_y, parent.max_x, parent.max_y, parent.max_size, parent.nodes)
                        parent._do_contraction()
                        break
                if parent is None:
                    #expand the canvas and try to lay it out.
                    root = self.get_top_quad_node()
                    expanded_min_x, expanded_min_y, expanded_max_x, expanded_max_y = self.get_new_bounds(root.min_x, root.min_y, root.max_x, root.max_y, root.max_size, root.nodes)
                    overlapping = root.layout_node_list(expanded_min_x, expanded_min_y, expanded_max_x, expanded_max_y, root.max_size, root.nodes)
                ...
                overlapping = self.layout_node_list(self.min_x, self.min_y, self.max_x, self.max_y, self.max_size, self.nodes)
                #print ("Should Fit")
    ...
    def quad_stats(self):
        ...
        if not has_children:
            ...
            if len(self.nodes) > self.total_cells:
                ...
                num_quad_fits = 1
        return num_quad_no_kids, num_quad_to_dense, num_quad_fits, total_nodes_moved, not_first_choice, lowest_level, max_nodes_in_grid
        ...
        if not has_children:
            for idx, node_a in enumerate(self.nodes):
                for node_b in self.nodes[:idx]:
                    if is_overlap(node_a.x, node_a.y, node_a.size, node_b.x, node_b.y, node_b.size):
    ...
    def get_nodes_near_lines(self, all_nodes):
        nodes_just_outside = []
        for n in all_nodes:
            if self.is_just_outside_box(self.min_x, self.min_y, self.max_x, self.max_y, self.max_size, n.x, n.y, n.size):
    ...
    def _do_contraction(self):
        ...
        nodes_by_size = sorted(node_list, key=lambda n: n.size, reverse=True)
        num_nodes = len(node_list)
        nodes_around_the_edge = self.get_nodes_near_lines(self.get_top_quad_node().nodes)
        ...
        for idx, node_to_move in enumerate(nodes_by_size):
            ...
            ov_idx = 0
            ov_idx, overlapping_node = is_overlapping_any_node_and_index(node_to_move, node_to_move.original_x, node_to_move.original_y, nodes_around_the_edge + nodes_by_size , ov_idx, idx + num_nodes_around_edge)
            ...
            while overlapping_node is not None and node_to_move.x != node_to_move.original_x:
                # slope doesn't change leave as original_xy
                slope_ca = (node_to_move.y - node_to_move.original_y)/(node_to_move.x - node_to_move.original_x)
                if node_to_move.nid == overlapping_node.nid:
                    raise Exception('They should not be the same node!! %s' %(node_to_move.nid))
                if node_to_move.original_x == new_x:
                    new_x += epsilon
                a = dist_original_to_over = distance.euclidean([node_to_move.original_x, node_to_move.original_y], [overlapping_node.x, overlapping_node.y])
                b = dist_from_original_to_new = distance.euclidean([node_to_move.original_x, node_to_move.original_y], [new_x, new_y])
                c = dist_from_new_to_overlapping = distance.euclidean([new_x, new_y], [overlapping_node.x, overlapping_node.y])
                ...
                new_len_b = len_c_new*math.sin(math.radians(angle_b_new))/math.sin(angle_c)
                ...
                x_new_plus = node_to_move.original_x + 	math.sqrt(new_len_b**2/(1+slope_ca**2))
                x_new_neg = node_to_move.original_x - math.sqrt(new_len_b**2/(1+slope_ca**2))
                ...
                ov_idx, overlapping_node = is_overlapping_any_node_and_index(node_to_move, new_x, new_y, nodes_around_the_edge + nodes_by_size, 0,idx+num_nodes_around_edge)
...
class quad_tree:
    ...
    def tree_stats(self):
        ...
        return list(results) + [results[3]/len(self.nodes), results[4]/len(self.nodes), self.root.sq_ratio]
