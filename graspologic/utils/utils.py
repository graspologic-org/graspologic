...
def to_weighted_edge_list(
    ...
    if isinstance(graph, list):
        ...
        if not isinstance(graph[0], tuple) or len(graph[0]) != 3:
            raise TypeError("If the provided graph is a list, it must be a list of tuples with 3 values in the form "
                            f"of Tuple[Any, Any, Union[int, float]], you provided {type(graph[0])}, {repr(graph[0])}")
        return [(str(source), str(target), float(weight)) for source, target, weight in graph]

    if isinstance(graph, nx.Graph):
        # will catch all networkx graph types
        try:
            return [
                (str(source), str(target), float(data.get(weight_attribute, weight_default)))
            ...
            raise ValueError(f"The networkx graph provided did not contain a {weight_attribute} that could be cast "
                             f"to float in one of the edges")

    if isinstance(graph, (np.ndarray, scipy.sparse.csr.csr_matrix)):
        shape = graph.shape
        if len(shape) != 2 or shape[0] != shape[1]:
            raise ValueError("graphs of type np.ndarray or csr.sparse.csr.csr_matrix should be adjacency matrices with "
                             "n x n shape")
        ...
        if not is_weighted and weight_default is None:
            raise ValueError("the adjacency matrix provided is not weighted and a default weight has not been set")
        ...
        if is_directed:
            for i in shape[0]:
                for j in shape[1]:
                    weight = graph[i][j]
                    if weight != 0:  # relying on a 0 here, which may be questionable
    ...
    raise TypeError(f"The type of graph provided {type(graph)} is not a list of 3-tuples, networkx graph, "
